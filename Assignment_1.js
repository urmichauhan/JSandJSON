/*2) Consider the following JSON dataset - 
 ...
 This dataset contains information of all characters (pokemon) of a famous game Pokemon Go. Include this dataset in a JS file as a
  letiable and then write the following functions - */

//JSON dataset of pokemon Go.
...
let checkstage = function(pname,pgame)
{
   for(let thepokemon in pgame.pokemon)
   {
      if(pgame.pokemon.next_evolution[name] == pname)
      {
        for(k of thepokemon)
        {
          alert(`${k}\n`);
...
} //checkstage function closed

/*Write a function that take a “Weakness” as an input and gives the names of all pokemon 
who have that Weakness. (check the weakness array in the dataset) */

let weaknames = function(weak,allpokemon)
{
   for(weakpokemon in allpokemon.pokemon)
   {
      let wlen = allpokemon.pokemon.length;
      for(k=0;k<wlen;k++)
      {
         if(allpokemon.pokemon[weakpokemon].weaknesses[k] == weak)
         {
            alert(`${allpokemon.pokemon[weakpokemon].name}\n`);
         }
      }
   }
} //weaknames function closed

let takename = prompt("Enter name of pokemon : ");
//call checkname function
checkname(takename,pokemongame);

let takenextname = prompt("Enter name of Pokemon : ");
//call checkstage function
checkstage(takenextname,pokemongame);

let takeweakness = prompt("Enter weakness name :");
//call weaknames function
weaknames(takeweakness,pokemongame);

alert('Thank You edWisor !');